...
module.exports = function (sails, dir, cb) {
  ...
  includeAll.optional({
    ...
    if (err) {
      console.log(colors.red('Failed to load plugin\'s controllers'));
      console.log(err);
      return cb(err);
    }
    ...
    try {
        ...
        if (match) {
          ...
          _.each(moduleDef, function (action, actionName) {
            ...
            if (actionsLoadedFromDisk[actionIdentity]) {
              ...
              }, new Error('The action `' + actionName + '` in `' + filePath + '` conflicts with a previously-loaded action.'));
            }
        ...
        // a function as its `fn` property.
          ...
          if (actionsLoadedFromDisk[actionIdentity]) {
            throw flaverr({
              ...
              code: 'E_CONFLICT',
              identity: actionIdentity
            }, new Error('The action `' + _.last(actionIdentity.split('/')) + '` in `' + filePath + '` conflicts with a previously-loaded action.'));
          }
      ...
      if (garbage.length) {
        ...
        _.each(garbage, function (filePath) {
          sails.log.warn('- ' + filePath);
        });
        sails.log.warn('----------------------------------------------------------------------------\n');
      }
      ...
      console.log(colors.red('Error occurred while loading plugin\'s controllers'));
      console.log(e);
      return cb(e);
    }
      ...
      _.each(_.map(target.split(','), _.trim), function (actionGlob) {
        ...
        warning += (function () {
          var fnDescs = _.reduce(fns, function (memo, fn) {
            if (fn._middlewareType) {
              memo.push(fn._middlewareType);
            }
            return memo;
          }, []);
          if (fnDescs.length) {
